<script>
	export default {
		...
		computed: {
			...
			imageSource() {
				let source = '';
				if (this.image.isArray() && this.image.length) {
					source = this.image[0]?.url;
				} else if (typeof this.image === 'object' && this.image !== null && 'url' in this.image) {
			...
			altText() {
				...
				} else if (typeof this.image === 'object' && this.image !== null && 'alt' in this.image) {
					text = this.image.alt;
				}
				return text;
			}
